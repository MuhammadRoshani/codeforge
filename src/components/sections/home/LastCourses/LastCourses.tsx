...
// Defines the teacher information required by the course card.
interface LatestCourseTeacher {
  name?: string | null;
}

// Defines the course fields returned by the latest courses API.
interface LatestCourse {
  ...
  teacher?: LatestCourseTeacher | null;
...
interface LatestCoursesResponse {
...
export default function LastCourses() {
  const [courses, setCourses] = useState<LatestCourse[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchLatestCourses = async () => {
      try {
        const { data } = await api.get<LatestCoursesResponse>(
          "/courses/latest",
          {
            timeout: 10000,
          },
