...
export default function CourseComments({
  ...
  const handleSubmitComment = async (
    ...
    try {
      const { data } = await api.post<CommentSubmitResponse>(
        `/courses/${course.slug}/comment`,
        {
          text: trimmedText,
        },
      );
   ...
   */
  const replyMap = comments.reduce<Record<string, CourseComment[]>>(
    (map, comment) => {
