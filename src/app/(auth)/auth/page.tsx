...
// Response returned by the OTP send endpoint.
interface SendOtpResponse {
  success: boolean;
  message?: string;
}

// Response returned by the OTP verification endpoint.
...
export default function AuthPage() {
  ...
  const handleSendOtp = async (e: SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      const { data } = await api.post<SendOtpResponse>("/auth/sms/send", {
        phone,
      });
  ...
  const handleVerifyOtp = async (e: SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const { data } = await api.post<VerifyOtpResponse>("/auth/sms/verify", {
        phone,
        otpCode,
      });
