...
const validatePhoneNumber = (phone: string): boolean => /^09\d{9}$/.test(phone);

// Validate the OTP format before querying the database.
const validateOtpCode = (code: string): boolean => /^\d{5}$/.test(code);
...
export async function POST(req: Request) {
  try {
    ...
    let requestBody: VerifyOtpRequestBody;

    try {
      requestBody = (await req.json()) as VerifyOtpRequestBody;
    } catch {
      return NextResponse.json(
        {
          success: false,
