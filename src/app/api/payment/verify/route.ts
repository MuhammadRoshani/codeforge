...
// Request body received from the payment callback.
interface PaymentVerificationBody {
  authority?: unknown;
  status?: unknown;
}

// Expected response shape returned by ZarinPal after payment verification.
...
export async function POST(req: Request) {
  try {
    ...
    let requestBody: PaymentVerificationBody;

    try {
      requestBody = (await req.json()) as PaymentVerificationBody;
    } catch {
    ...
    const { authority, status } = requestBody;
    ...
    const result = (await response.json()) as ZarinPalVerificationResponse;
