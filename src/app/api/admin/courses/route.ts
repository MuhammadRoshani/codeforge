...
// MongoDB query structure used by the admin course search.
type CourseQuery =
  | Record<string, never>
  | {
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (search) {
      ...
      const statusMap: Record<"draft" | "published" | "coming-soon", string[]> =
        {
          published: [
            "published",
            "publish",
      ...
      let matchedStatus: keyof typeof statusMap | null = null;
      ...
      for (const [status, keywords] of Object.entries(statusMap)) {
        if (keywords.includes(normalizedSearch)) {
          matchedStatus = status as keyof typeof statusMap;
          break;
        }
      }
