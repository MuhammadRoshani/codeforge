...
export async function PUT(
  ...
  try {
    ...
    if (chaptersValidation.error || !chaptersValidation.chapters) {
      return validationErrorResponse(
        NextResponse,
        chaptersValidation.error || "Invalid chapters data.",
      );
    }
    ...
    // The validation above guarantees that support is one of the valid Course values.
    course.support = support as CourseType["support"];

    course.price = numericPrice;
    course.discountPrice = isFree ? null : numericDiscountPrice;
    course.isFree = isFree;

    // The validation above guarantees that level is one of the valid Course values.
    course.level = level as CourseType["level"];

    // The validation above guarantees that status is one of the valid Course values.
    course.status = status as CourseType["status"];
