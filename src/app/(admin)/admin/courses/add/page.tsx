...
type Status = "draft" | "published" | "coming-soon";
type SupportType = "ticket" | "q&a" | "telegram" | "whatsapp";
...
type SelectType = "level" | "teacher" | "status" | "support";
 ...
 */
const SUPPORT_TYPES: SupportType[] = ["ticket", "q&a", "telegram", "whatsapp"];
 ...
 */
const createEmptyLesson = (): LessonFormData => ({
  title: "",
  duration: "",
  isFree: false,
  videoUrl: "",
 ...
 */
const createEmptyChapter = (): ChapterFormData => ({
  title: "",
...
export default function AddCourse() {
  ...
  const [openSelect, setOpenSelect] = useState<SelectType | null>(null);
  ...
  useEffect(() => {
    const fetchTeachers = async () => {
      try {
        ...
        const response = await api.get<TeachersResponse>("/admin/courses/add");
   ...
   */
  const updateLesson = <K extends keyof LessonFormData>(
    chapterIndex: number,
    lessonIndex: number,
    field: K,
   ...
   */
  const removeChapter = async (chapterIndex: number) => {
   ...
   */
  const addCourseHandler = async (e: SubmitEvent<HTMLFormElement>) => {
    e.preventDefault();
