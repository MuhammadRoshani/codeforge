...
// Defines the course fields required by the admin course management page.
...
// Defines the API response returned by the admin courses endpoint.
...
// Defines the API error response used when deleting a course.
interface DeleteCourseResponse {
  success: boolean;
  message?: string;
}

// Defines the message types displayed on the page.
...
export default function Courses() {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const { data } = await api.get<CoursesResponse>("/admin/courses", {
          params: {
  ...
  const deleteCourseHandler = async (slug: string) => {
