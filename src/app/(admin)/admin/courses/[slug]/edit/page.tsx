...
const SUPPORT_TYPES = ["ticket", "q&a", "telegram", "whatsapp"] as const;
...
interface CourseFromApi {
  ...
  thumbnail?: string | ThumbnailObject | null;
  chapters?: Chapter[];
}
...
const normalizeThumbnailUrl = (
  thumbnail: string | ThumbnailObject | null | undefined,
): string | null => {
  if (!thumbnail) {
    return null;
  }
...
const createEmptyChapter = (): Chapter => ({
  title: "",
...
export default function EditCourse() {
  ...
  const [openSelect, setOpenSelect] = useState<
    "level" | "teacher" | "status" | "support" | null
  >(null);
   ...
   */
  const editCourseHandler = async (e: SubmitEvent<HTMLFormElement>) => {
    e.preventDefault();
    ...
    data.append(
      "discountPrice",
      formData.isFree ? "" : String(formData.discountPrice),
    );
