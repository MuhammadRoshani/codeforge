...
// Represents the user fields returned by the admin users API.
interface AdminUser {
  _id: string;
  phone: string;
  email: string | null;
...
// Represents the editable fields of the selected user.
...
// Response returned by the admin users API.
...
// Response returned when fetching users fails.
interface FetchUsersErrorResponse {
  success: false;
  message: string;
}

type FetchUsersApiResponse = FetchUsersResponse | FetchUsersErrorResponse;

// Response returned after successfully updating a user.
...
// Response returned when updating a user fails.
interface UpdateUserErrorResponse {
  success: false;
  message: string;
}

type UpdateUserApiResponse = UpdateUserResponse | UpdateUserErrorResponse;
...
export default function AdminUsers() {
  ...
  const fetchUsers = async (searchQuery: string): Promise<void> => {
    try {
      const { data } = await api.get<FetchUsersApiResponse>("/admin/users", {
        params: {
          search: searchQuery,
          page,
        },
   ...
   */
  const editUserHandler = async (id: string): Promise<void> => {
    ...
    try {
      const { data } = await api.patch<UpdateUserApiResponse>(
        `/admin/users/${id}`,
        updatedForm,
      );
